...
class IROSAutomation {
    ...
    async selectRegistryItems(){
        ...
        // 새 화면의 '미공개' 버튼을 기다립니다.
        await this.page.getByRole('radio', { name: '미공개' }).waitFor();
...
// ▲▲▲▲▲ [끝] 여기까지 교체 ▲▲▲▲▲
    ...
    async setPrivacyOption(){
        ...
        // ✨ 1. '미공개' 버튼이 나타나고 안정될 때까지 명시적으로 기다립니다.
        const privateRadio = this.page.getByRole('radio', { name: '미공개' });
        await privateRadio.waitFor();
    
        // ✨ 2. 기다림이 끝난 후, 버튼을 체크(check)합니다.
        await privateRadio.check();
        
        // ✨ 3. 실제로 체크되었는지 '검증'하는 로직을 추가합니다.
       if (await privateRadio.isChecked()) {
          console.log('✅ "미공개" 라디오 버튼이 성공적으로 체크되었습니다.');
       } else {
          console.log('❌ "미공개" 라디오 버튼 체크 실패!');
          // 에러를 발생시켜 스크립트를 중단시킵니다.
            ...
            try {
                ...
                for (let j = 0; j < batch.length; j++) {
                    ...
                    if (result === true) {
                        ...
                        failCount++;
                        console.log(`❌ "${companyName}" 실패 (${failCount}개 실패)`);
                    }
                }
