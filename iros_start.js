...
async function viewCertificateResult(page){
  ...
  try {
    ...
    console.log('✅ PDF 콘텐츠 로드 완료 ("등기사항전부증명서" 텍스트 감지)');
      ...
      await page.screenshot({ path: `error_view_certificate_${Date.now()}.png` });
      throw error;
  }
}
// pdf 읽기 및 이름 변경 함수//
async function readAndRenamePdfFile(downloadFolder){
}
(async () => {
  ...
  //다운로드 경로 설정
  const downloadFolder = path.join(process.cwd(), 'download_folder');
