...
async function selectCertificateType(page){
  ...
  async function selectSpecificItemsAndProceed(page) {
    ...
    try {
        ...
        throw new Error('특정 등기 항목 선택 및 진행에 실패했습니다.');
    }
}
...
(async () => {
  ...
  // --- 2. 로그인 완료 확인 (추가된 부분) ---
  // '로그아웃' 버튼이 화면에 나타날 때까지 기다려서 로그인이 확실히 끝났는지 확인합니다.
  console.log('로그인 완료 확인 중...');
  await page.getByRole('link', { name: '로그아웃' }).first().waitFor();
  console.log('로그인 성공! 메인 페이지로 돌아왔습니다.');
  try {
  ...
  // --- 3. 검색 함수 호출 ---// 
  await searchBusinessByName(page,companyObjects[0].등기상호);
  // ---4.법인 등기사항 증명서 열람·발급 신청---//
  await issueCertificate(page)
  // ---5. 등기용도 및 등기유형 선택---//
