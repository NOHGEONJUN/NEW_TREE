...
const LOGIN_ID = 'shrjswns';
const LOGIN_PASSWORD = 'dabin58781!';
const EMONEY_NUMBER_FRONT = 'L13678818';
const EMONEY_NUMBER_BACK = '9465';
const EMONEY_PASSWORD = 'skku1234';

// 엑셀 파일 로딩 및 객체 처리 함수(같은 폴더에 information.xlsx 파일이 있어야 함) //
async function getCompanyObjects(filePath = './information.xlsx') {
  try {
    // 1. Excel 파일 읽기 (버퍼로 읽기)
    ...
    // 4. 시트 → 2차원 배열 변환 (CSV와 비슷한 구조)
    const records = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', blankrows: false });
    ...
    // 7. 2차원 배열 → 객체 배열 변환
    const companyObjects = dataRows.map(row =>
...
async function searchBusinessByName(page, businessName) {
  ...
  console.log(`--- [검색 완료] "${businessName}" 결과 확인! ---`);
    ...
    throw new Error(`검색 실패: ${businessName}`);
  }
}
...
async function selectPaymentMethod(page){
  ...
  try {
    ...
    await page.locator(emoneyInputSelector_front).fill(EMONEY_NUMBER_FRONT)
    console.log('성공적으로 선불전자지급수단번호 앞자리를 입력했습니다')
    await page.locator(emoneyInputSelector_back).fill(EMONEY_NUMBER_BACK)
    console.log('성공적으로 선불전자지급수단번호 뒷자리를 입력했습니다')
    await page.locator(passwordInputSelector).fill(EMONEY_PASSWORD);
...
async function processPdfAndRename(folderPath = './iros_downloads') {
  try {
    ...
    const companies = companyMatch[1].match(/주식회사\s+[가-힣0-9·\-\&]+/g);
...
// 실패목록 엑셀 파일 생성 함수수
async function saveFailedCompaniesToExcel(failedCompanies){
   // 날짜별 파일명 생성 (예: failed_companies_2025-10-29.xlsx)
   const now = new Date();
   const datePart = now.toISOString().split('T')[0]; // YYYY-MM-DD
   const timePart = now.toTimeString().split(' ')[0].replace(/:/g, '-'); // HH-MM-SS
   const fileName = `failed_companies_${datePart}_${timePart}.xlsx`; // 예: failed_companies_2025-10-29_15-37-08.xlsx

  // 엑셀 데이터 구성
  const worksheetData = failedCompanies.map(name => ({ '실패한 상호명': name }));
  const worksheet = XLSX.utils.json_to_sheet(worksheetData);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, 'FailedCompanies');

  // 파일 저장
  XLSX.writeFile(workbook, fileName);
  console.log(`📄 실패 목록이 엑셀로 저장되었습니다: ${fileName}`);
}
...
(async () => {
  ...
  await page.locator('input[title="아이디입력"]:visible').fill(LOGIN_ID);
  await page.locator('input[title="비밀번호입력"]:visible').fill(LOGIN_PASSWORD);
  ...
  if (failCount > 0) {
    ...
    //실패기업 엑셀 저장
    await saveFailedCompaniesToExcel(failedCompanies);
}
})();