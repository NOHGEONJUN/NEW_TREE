...
class IROSAutomation {
    ...
    async setIssuanceOptions() {
        ...
        // 🎯 열람 라디오 버튼 선택 (JavaScript 방법만 사용)
    ...
    async selectRegistryItems() {
        ...
        try {
            ...
            console.log('🔍 클래스+data-rowindex 방식으로 체크박스 찾는 중...');
            await this.page.waitForSelector('input.w2grid_embedded_check[data-rowindex="14"]', { timeout: 10000 });
            await this.page.waitForSelector('input.w2grid_embedded_check[data-rowindex="15"]', { timeout: 10000 });
            ...
            console.log('⚠️ 클래스 방식 실패:', e1.message);
            console.log('🔍 정확한 ID 방식으로 체크박스 찾는 중...');
            
            try {
                // 2순위: 정확한 ID 사용
                await this.page.waitForSelector('#G_mf_wfm_potal_main_wfm_content_grd_item_sel_obj_list___checkbox_dynamic_checkbox_14_0_14', { timeout: 10000 });
                await this.page.waitForSelector('#G_mf_wfm_potal_main_wfm_content_grd_item_sel_obj_list___checkbox_dynamic_checkbox_15_0_15', { timeout: 10000 });
                ...
                console.log('⚠️ ID 방식도 실패:', e2.message);
                console.log('🔍 JavaScript 방법으로 체크박스 찾는 중...');
                
                // 3순위: JavaScript evaluate 방식 (더 안전한 방법)
                const result = await this.page.evaluate(() => {
                    let checkedCount = 0;
                    let foundElements = [];
                    ...
                    console.log(`🔍 페이지에서 ${allCheckboxes.length}개의 체크박스 발견`);
                    
                    // data-rowindex로 찾기
                    ['14', '15'].forEach(rowIndex => {
                        const checkbox = document.querySelector(`input[data-rowindex="${rowIndex}"]`);
                        if (checkbox) {
                            foundElements.push(`data-rowindex="${rowIndex}"`);
                            if (!checkbox.checked) {
                                checkbox.click();
                                checkedCount++;
                                console.log(`✅ data-rowindex="${rowIndex}" 체크박스 선택됨`);
                            } else {
                                console.log(`ℹ️ data-rowindex="${rowIndex}" 체크박스는 이미 선택됨`);
                            }
                        } else {
                            console.log(`❌ data-rowindex="${rowIndex}" 체크박스를 찾을 수 없음`);
                        }
                    });
                ...
                console.log('JavaScript 실행 결과:', result);
                if (result.checkedCount === 0 && result.foundElements.length === 0) {
                    console.log('⚠️ 체크박스를 찾을 수 없습니다. 페이지 상태를 확인해주세요.');
                }
            }
        }
